...
class EuclideanDistTracker:
    ...
    def capture(self,img,x,y,h,w,sp,id):
        if(self.capf[id]==0):
            ...
            #'D://TrafficRecord//' + n + '.jpg'
            cv2.imwrite(file, crop_img)
            self.count += 1
             #open("../TrafficRecord//SpeedRecord.txt","w")
            ...
            if(sp>limit):
                ...
                try:   
                    ...
                    query = "INSERT INTO cars_information(id, speed,notice) VALUES(%s,%s,%s)"
                    values=(id,sp,"Exceeded")
                    #"insert into cars_information(id,speed,status,notice) values(id,sp,0,\"Exceeded\")"
                ...
                try:   
                    mycursor = mydb.cursor()
                    query = "INSERT INTO cars_information(id, speed,status,notice) VALUES(%s,%s,%s,%s)"
                    values=(id,sp,1,"Normal")
                    #"insert into cars_information(id,speed,status,notice) values(id,sp,0,\"Exceeded\")"
                    mycursor.execute(query,values)
                    # Commit your changes in the database
                    mydb.commit()
